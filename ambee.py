...
def log(value, status='NTB', color='[P1]'):
...
def log_replace(value, status='NTB', color='[Y1]'):
    ...
    def start(self):
        try:
            ...
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check frontend-domains.txt', color='[Y1]')
                return
            self.log('P & P {} port {}'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(4096)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('NTB {} port {}'.format(self.inject_host, self.inject_port), color='[G1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='NTB', color='[R1]'):
